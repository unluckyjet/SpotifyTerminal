...
export class PlayerUI {
  ...
  draw(track:Track, artwork:Artwork|undefined, demo:boolean, status:string) {
    ...
    if(W<32||H<16){
      ...
      centered(track.name,top);
      centered(track.album,top+1,colors.muted);
      centered(track.artist,top+2,colors.accent);
      const ax=Math.floor((W-artW)/2),ay=top+4;
      this.cover.left=ax;
      this.cover.top=ay;
      ...
      put(left,controlY,clock(track.position),colors.muted);
      ...
      button(center-spacing-1,' ← ','previous');
      button(center-1,track.playing?' Ⅱ ':' ▶ ','toggle');
      button(center+spacing-1,' → ','next');
      const note=status||this.overlay?.note|| (demo?'demo · no audio':'');
    ...
    for(let y=0;y<H;y++){
      if(!this.rows[y]){const row=new TextRenderable(this.renderer,{id:`row-${y}`,position:'absolute',left:0,top:y,width:W,height:1,onMouseDown:e=>{const h=this.hits.find(h=>e.y===h.y&&e.x>=h.x&&e.x<h.x+h.w);if(h)this.action(h.command);}});this.rows.push(row);this.renderer.root.add(row);}
      const chunks:any[]=[];
