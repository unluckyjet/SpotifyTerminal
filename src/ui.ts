...
export class PlayerUI {
  ...
  draw(track:Track, artwork:Artwork|undefined, demo:boolean, status:string) {
    ...
    const fullscreen=this.fullscreen&&!this.gallery&&!this.lyricsOpen;
    ...
    if(W<32||H<16){
      ...
      const lyrics=this.lyricsOpen&&!this.gallery;
      ...
      const ax=sideLyrics?Math.max(2,Math.floor((W*0.48-artW)/2)):Math.floor((W-artW)/2),ay=top+(fullscreen?0:4);
      this.cover.left=ax;
      this.cover.top=ay;
      ...
      if(lyrics){
        const index=lyricIndex(this.lyricLines,track.position),count=Math.max(1,Math.min(5,Math.floor((artH+1)/2))),start=Math.max(0,index-Math.floor(count/2));
        const lx=sideLyrics?Math.floor(W*0.53):left,lw=sideLyrics?W-lx-3:width,ly=ay+Math.max(0,Math.floor((artH-(count*2-1))/2));
        if(!this.lyricLines.length){put(lx,ly,'No lyrics for this track'.slice(0,lw),colors.muted);put(lx,ly+2,'I import a .lrc file'.slice(0,lw),colors.accent);}
